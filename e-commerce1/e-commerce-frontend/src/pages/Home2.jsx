...
function Home2() {
  ...
  const getImgSrc = (p) =>
    p?.imagenUrl
      ? `http://localhost:8080/imagenes/${p.imagenUrl}`
      : 'https://via.placeholder.com/300x400?text=E40';
  ...
  return (
    <div>
      ...
      <section style={heroStyle}>
        ...
        <div style={heroVisual}>
          <div style={collageWrapper}>
            ...
            <div style={card1Style}>
              <div style={cardInner}>
                <img
                  src={fotosHero[0] ? getImgSrc(fotosHero[0]) : 'https://via.placeholder.com/190x260?text=Prenda'}
                  alt={fotosHero[0]?.nombre || 'Prenda'}
                  style={cardImgStyle}
                  onError={(e) => { e.target.src = 'https://via.placeholder.com/190x260?text=E40'; }}
                />
                <div style={cardLabel}>
                  <span style={cardCat}>{fotosHero[0]?.categoria?.nombreCategoria || 'Remeras'}</span>
                  <span style={cardName}>{fotosHero[0]?.nombre || 'Camisa Oxford'}</span>
                </div>
              </div>
            </div>
            ...
            <div style={card2Style}>
              <div style={cardInner}>
                <img
                  src={fotosHero[1] ? getImgSrc(fotosHero[1]) : 'https://via.placeholder.com/210x280?text=Prenda'}
                  alt={fotosHero[1]?.nombre || 'Prenda'}
                  style={cardImgStyle}
                  onError={(e) => { e.target.src = 'https://via.placeholder.com/210x280?text=E40'; }}
                />
                <div style={cardLabel}>
                  <span style={cardCat}>{fotosHero[1]?.categoria?.nombreCategoria || 'Pantalones'}</span>
                  <span style={cardName}>{fotosHero[1]?.nombre || 'Jean Clásico'}</span>
                </div>
              </div>
            </div>
            ...
            <div style={card3Style}>
              <div style={cardInner}>
                <img
                  src={fotosHero[2] ? getImgSrc(fotosHero[2]) : 'https://via.placeholder.com/230x175?text=Prenda'}
                  alt={fotosHero[2]?.nombre || 'Prenda'}
                  style={cardImgStyle}
                  onError={(e) => { e.target.src = 'https://via.placeholder.com/230x175?text=E40'; }}
                />
                <div style={cardLabel}>
                  <span style={cardCat}>{fotosHero[2]?.categoria?.nombreCategoria || 'Pantalones Cortos'}</span>
                  <span style={cardName}>{fotosHero[2]?.nombre || 'Malla Floreada'}</span>
                </div>
              </div>
            </div>
        ...
        <div style={heroScrollHint}>
          <div style={heroScrollLine}></div>
          <span style={heroScrollText}>Scroll</span>
        </div>
      </section>
                ...
                productosFiltrados.map(p => (
                  <div key={p.id} style={cardStyle}>
                    <div style={imgContainer}>
                      <img
                        src={p.imagenUrl ? `http://localhost:8080/imagenes/${p.imagenUrl}` : 'https://via.placeholder.com/300?text=SIN+IMAGEN'}
                        alt={p.nombre}
                        style={imgStyle}
                        onError={(e) => { e.target.src = 'https://via.placeholder.com/300?text=ESTUDIO+40'; }}
                      />
                    </div>
                    <div style={{ textAlign: 'left', width: '100%', paddingLeft: '10px' }}>
                      <span style={catLabelStyle}>{p.categoria?.nombreCategoria || 'General'}</span>
