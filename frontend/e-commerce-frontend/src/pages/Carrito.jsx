...
function Carrito() {
  ...
  const finalizarCompra = async () => {
    ...
    try {
      await checkoutCarrito(usuario.id).unwrap();

      alert("¡Compra realizada con éxito! Tu orden ha sido archivada.");
      navigate('/');
    } catch (error) {
      console.error("Error en el checkout:", error);
      const mensajeError = error.data || "Error al procesar la compra. Verifica el stock.";
      alert(mensajeError);
    }
  };

  const quitarItem = async (item, index) => {
    try {
      await dispatch(eliminarDelCarritoAsync({
        itemId: item.carritoItemId,
        usuarioId: usuario.id,
      })).unwrap();
    } catch (error) {
      alert(error || "No se pudo quitar el producto.");
    }
  };

  const vaciar = async () => {
    try {
      await dispatch(vaciarCarritoAsync(usuario.id)).unwrap();
    } catch (error) {
          ...
          {items.length > 0 ? (
            <>
              {items.map((item, index) => (
                <div key={item.carritoItemId || index} style={itemContainer}>
                  <img 
                    src={item.imagenesUrl?.[0] ? `http://localhost:8080/imagenes/${item.imagenesUrl[0]}` : 'https://placehold.co/100?text=ESTUDIO+40'}
                    alt={item.nombre} 
                    style={miniImg} 
